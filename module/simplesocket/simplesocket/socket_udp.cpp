...
namespace module {
    ...
    CUDPSocket::CUDPSocket() : __sConnect(INVALID_SOCKET) {}
    ...
    int CUDPSocket::Create(const char *pip/* = NULL*/, uint16 uport/* = 0*/, bool bclient/* = true*/) {
        if ((__sConnect = CBaseSocket::Create(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == INVALID_SOCKET) {
            perror("CreateServer udp socket failed!");
            return -1;
        }
        ...
        // Set port reuse
        if (CBaseSocket::SetSocketOption(__sConnect, SOL_SOCKET, SO_REUSEADDR, (char *) &on, sizeof(int)) == -1) {
        ...
        if (CBaseSocket::Bind(__sConnect, (sockaddr *) &sin, sizeof(sin)) == SOCKET_ERROR) {
            perror("Bind failed!");
            return -1;
        }
    ...
    int CUDPSocket::Read(void *pout, size_t szout, SOCKADDR &addr) {
        return CBaseSocket::RecvFrom(__sConnect, pout, szout, 0, addr);
    }
    ...
    int CUDPSocket::Write(const void *pin, size_t szin, const char *pip, uint16 uport) {
        ...
        return CBaseSocket::SendTo(__sConnect, pin, szin, 0, *(SOCKADDR *) &addr);
    }
    ...
    int CUDPSocket::Write(const void *pin, size_t szin, SOCKADDR addr) {
        return CBaseSocket::SendTo(__sConnect, pin, szin, 0, addr);
    }
